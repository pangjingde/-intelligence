...
public class CartServiceImpl implements ICartService {
    ...
    public List<CartVO> getVoByUid(Integer uid) {
        List<CartVO> vo= cartMapper.findVoByUid(uid);
        return vo;
    }
    ...
    @Override
    public Integer decrease(Integer cid, Integer uid, String username) {
        Cart result = cartMapper.findByCid(cid);
        if (result==null){
    ...
    @Override
    public Integer delete(Integer cid, Integer uid, String username) {
        Cart result = cartMapper.findByCid(cid);
        if (result==null){
            throw  new CartNotFoundException("购物车数据不存在");
        }if (!result.getUid().equals(uid)){
        ...
        if (rows!=1){
            throw  new DeleteException("删除购物车数据出错");
        }
        return 1;
    }
